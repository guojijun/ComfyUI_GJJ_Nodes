...
function currentSize(node) {
	return {
		width: Math.max(16, Number.parseInt(GJJ_Utils.getWidget(node, WIDTH_WIDGET)?.value, 10) || 16),
		height: Math.max(16, Number.parseInt(GJJ_Utils.getWidget(node, HEIGHT_WIDGET)?.value, 10) || 16),
	};
}
...
function applySize(node, width, height) {
	setWidgetValue(GJJ_Utils.getWidget(node, WIDTH_WIDGET), roundToMultipleOfEight(width));
	setWidgetValue(GJJ_Utils.getWidget(node, HEIGHT_WIDGET), roundToMultipleOfEight(height));
}
			...
			if (Number.isFinite(width) && Number.isFinite(height)) {
				...
				try {
					...
					if (Number.isFinite(batchSize)) {
						setWidgetValue(GJJ_Utils.getWidget(this, BATCH_WIDGET), batchSize);
					}
					if (aspectRatio && GJJ_Utils.getWidget(this, ASPECT_RATIO_WIDGET)) {
						setWidgetValue(GJJ_Utils.getWidget(this, ASPECT_RATIO_WIDGET), aspectRatio);
					}
				} finally {

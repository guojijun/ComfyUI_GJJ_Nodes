...
app.registerExtension({
  name: "GJJ.LTXVSparseTrackEditor",

  async nodeCreated(node) {
    if (node.comfyClass !== "GJJ_LTXVSparseTrackEditor") return;
    initEditor(node);
  },

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData?.name !== "GJJ_LTXVSparseTrackEditor") return;
...
function render(node) {
  ...
  if (ed.splines.length > 1) {
    ...
    ctx.textAlign = "left";
    ctx.fillText(
      `样条 ${ed.active + 1} / ${ed.splines.length}`,
      10 * dpr,
      18 * dpr
    );
  }

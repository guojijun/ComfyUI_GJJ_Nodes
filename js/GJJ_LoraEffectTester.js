...
function readState(node) {
	if (!node.__gjjLoraEffectState) {
		node.__gjjLoraEffectState = parseState(
			node.properties?.[STATE_WIDGET] ?? GJJ_Utils.getWidget(node, STATE_WIDGET)?.value ?? "",
		);
	}
...
function currentIndex(node) {
	const value = Number(GJJ_Utils.getWidget(node, INDEX_WIDGET)?.value ?? 1);
...
function compactNode(node) {
	for (const name of OBSOLETE_WIDGETS) {
		hideWidget(GJJ_Utils.getWidget(node, name), false, "");
	}
		...
		nodeType.prototype.onConfigure = function (...args) {
			...
			setTimeout(() => {
				this.__gjjLoraEffectState = parseState(
					this.properties?.[STATE_WIDGET] ?? GJJ_Utils.getWidget(this, STATE_WIDGET)?.value ?? "",
				);
				compactNode(this);
				setupControls(this);
				setupPanel(this);

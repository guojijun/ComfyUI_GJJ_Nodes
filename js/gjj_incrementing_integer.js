...
function controlMode(node) {
	return String(findWidget(node, CONTROL_WIDGET)?.value || "").toLowerCase();
}

function countStep(node) {
	const raw = Number(findWidget(node, COUNT_WIDGET)?.value ?? 1);
...
function setDefaultIncrement(node) {
	...
	const control = findWidget(node, CONTROL_WIDGET);
	if (control) {
...
function compensateIncrementStep(promptId) {
	...
	for (const node of app.graph?._nodes || []) {
		...
		const valueWidget = findWidget(node, VALUE_WIDGET);

...
rn widget ? widget.value : fallback;
}
...
function setModelOptions(node, values) {
	const widget = GJJ_Utils.getWidget(node, MODEL_WIDGET);
...
function hideInternalWidgets(node) {
	hideWidget(GJJ_Utils.getWidget(node, DIRECTORY_WIDGET));
	hideWidget(GJJ_Utils.getWidget(node, SUMMARY_WIDGET));
}
...
function patchNode(node) {
	...
	const hostWidget = GJJ_Utils.getWidget(node, HOST_WIDGET);

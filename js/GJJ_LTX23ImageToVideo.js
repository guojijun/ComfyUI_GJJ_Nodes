...
function isAutoSizeEnabled(node) {
	return Boolean(GJJ_Utils.getWidget(node, AUTO_SIZE_WIDGET)?.value);
}
...
function applyAlignedSize(node, width, height) {
	...
	setWidgetValue(GJJ_Utils.getWidget(node, WIDTH_WIDGET), roundUpTo32(width));
	setWidgetValue(GJJ_Utils.getWidget(node, HEIGHT_WIDGET), roundUpTo32(height));
}
...
function applyFrameCount(node, frameCount) {
	...
	setWidgetValue(GJJ_Utils.getWidget(node, FRAME_COUNT_WIDGET), value);
}

...
} from "./GJJ_ModelFamilyPresetTable.js";
...
async function loadImageDimensionsFromSourceNode(sourceNode) {
	...
	if (sourceNode.comfyClass === "GJJ_MultiImageLoader") {
		...
		const selectedWidget = GJJ_Utils.getWidget(sourceNode, "selected_images");
		let selectedImages = [];
		try {
...
async function trySyncPrimaryImageSize(node) {
	...
	setWidgetValue(GJJ_Utils.getWidget(node, WIDTH_WIDGET), roundToMultipleOfEight(size.width));
	setWidgetValue(GJJ_Utils.getWidget(node, HEIGHT_WIDGET), roundToMultipleOfEight(size.height));
	GJJ_Utils.refreshNode(node);
}
...
function syncPrimaryBatchSource(node) {
	const widget = GJJ_Utils.getWidget(node, BATCH_SOURCE_IMAGES_WIDGET);
	if (!widget) {
		return;
	}
	const info = getPrimaryImageLinkInfo(node);
...
function renameImages(node) {
	...
	hideWidget(GJJ_Utils.getWidget(node, BATCH_SOURCE_IMAGES_WIDGET));
...
function updateClipWidgetVisibility(node, clipCount) {
	const clipWidget = GJJ_Utils.getWidget(node, "clip_name1");
	if (!clipWidget) {
		return;
	}
	const preset = matchPreset(GJJ_Utils.getWidget(node, "unet_name")?.value || "");
...
function updateMainImageIndexVisibility(node, preset = null) {
	const widget = GJJ_Utils.getWidget(node, "main_image_index");
	if (!widget) {
		return;
	}
	const equalReference = usesEqualReferenceCanvas(preset, GJJ_Utils.getWidget(node, "unet_name")?.value || "");
...
function updateLoraWidgetVisibility(node, preset = null) {
	...
	for (const row of rows) {
		const nameWidget = GJJ_Utils.getWidget(node, row.name);
		const strengthWidget = GJJ_Utils.getWidget(node, row.strength);
...
function syncStepsFromCurrentLoras(node) {
	const stepsWidget = GJJ_Utils.getWidget(node, "steps");
	if (!stepsWidget) {
		return;
	}
	const preset = matchPreset(GJJ_Utils.getWidget(node, "unet_name")?.value || "");
	const lora1Name = GJJ_Utils.getWidget(node, "lora_1_name")?.value || "";
	const lora2Name = GJJ_Utils.getWidget(node, "lora_2_name")?.value || "";
	const lora1Strength = GJJ_Utils.getWidget(node, "lora_1_strength")?.value || 0;
	const lora2Strength = GJJ_Utils.getWidget(node, "lora_2_strength")?.value || 0;
...
function applyPreset(node, force = false) {
	const unetWidget = GJJ_Utils.getWidget(node, "unet_name");
	if (!unetWidget) {
		return;
	}
	const preset = matchPreset(unetWidget.value);
	if (!preset) {
		setWidgetValueOrClear(GJJ_Utils.getWidget(node, "lora_1_name"), "");
		setWidgetValueOrClear(GJJ_Utils.getWidget(node, "lora_2_name"), "");
	...
	setWidgetValueOrClear(GJJ_Utils.getWidget(node, "vae_name"), preset.vaeName);
	setWidgetValueOrClear(GJJ_Utils.getWidget(node, "lora_1_name"), preset.lora1);
	setWidgetValue(GJJ_Utils.getWidget(node, "lora_1_strength"), preset.lora1Strength);
	setWidgetValueOrClear(GJJ_Utils.getWidget(node, "lora_2_name"), preset.lora2);
	setWidgetValue(GJJ_Utils.getWidget(node, "lora_2_strength"), preset.lora2Strength);
	setWidgetValue(GJJ_Utils.getWidget(node, "steps"), preset.steps);
	setWidgetValue(GJJ_Utils.getWidget(node, "cfg"), preset.cfg);
	setWidgetValue(GJJ_Utils.getWidget(node, "sampler_name"), preset.sampler);
	setWidgetValue(GJJ_Utils.getWidget(node, "scheduler"), preset.scheduler);
	setWidgetValue(GJJ_Utils.getWidget(node, "denoise"), preset.denoise);
	setWidgetValue(GJJ_Utils.getWidget(node, "width"), preset.width);
	setWidgetValue(GJJ_Utils.getWidget(node, "height"), preset.height);
...
function enhanceUnetWidget(node) {
	const widget = GJJ_Utils.getWidget(node, "unet_name");

...
function refreshNode(node) {
	const size = node.computeSize?.() || node.size || [320, 200];
	node.setSize?.([Math.max(size[0] || 320, 320), size[1] || 200]);
...
function updateVisibility(node) {
	const expandWidget = getWidget(node, EXPAND_WIDGET_NAME);
	const fillWidget = getWidget(node, FILL_WIDGET_NAME);
	...
	for (const name of MARGIN_ONLY_WIDGETS) {
		setWidgetEnabled(getWidget(node, name), expandMode === MODE_MARGINS);
	}
	for (const name of RATIO_ONLY_WIDGETS) {
		setWidgetEnabled(getWidget(node, name), expandMode === MODE_RATIO);
	}
	for (const name of COLOR_ONLY_WIDGETS) {
		setWidgetEnabled(getWidget(node, name), fillMode === FILL_BLACK);
	}

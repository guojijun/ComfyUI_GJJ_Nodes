...
const STATUS_ENABLED_CLASSES = new Set([
	...
	"GJJ_SaveRGBAAnimated",
	"GJJ_SAM2PointMaskEditor",
...
const LEGACY_STATUS_WIDGET_POLICIES = new Map([
	...
	["gjj_audio_status", "hide"],
	["gjj_batch_outpaint_status", "hide"],
	["gjj_checkpoint_status", "hide"],
	["gjj_flashvsr_status", "hide"],
	["gjj_lazy_image_status", "hide"],
	["gjj_multiview_status", "hide"],
	["gjj_old_photo_status", "hide"],
	["gjj_ollama_llm_status", "hide"],
	["gjj_rife_vfi_status", "hide"],
	["gjj_seedvr2_status", "hide"],
	["gjj_wan22_flf2v_status", "hide"],
	["gjj_wan22_rapid_aio_mega_status", "hide"],
	["gjj_audio_ace_music_status", "strip-progress"],
	["gjj_cosyvoice3_status", "strip-progress"],
	["gjj_fish_audio_s2_status", "strip-progress"],
	["gjj_longcat_status", "strip-progress"],
	["__gjj_faster_whisper_status", "strip-progress"],
	["__gjj_qwen3_asr_status", "strip-progress"],
	["__gjj_sense_voice_status", "strip-progress"],
	["gjj_save_rgba_animated_status", "strip-card"],
	["gjj_video_combine_status", "strip-card"],
]);
...
const LEGACY_ERROR_EVENTS = [
	"gjj_audio_separator_error",
	"gjj_batch_outpaint_error",
	"gjj_batch_text_error",
	"gjj_faster_whisper_error",
	"gjj_fish_audio_s2_error",
	"gjj_qwen3_error",
...
function suppressLegacyStatusWidget(node, widget) {
	if (
		String(node?.comfyClass || node?.type || "") === "GJJ_LoraFaceMaterialGenerator"
		&& String(widget?.name || "") === "状态"
	) {
		widget.hidden = true;
	...
	const policy = LEGACY_STATUS_WIDGET_POLICIES.get(String(widget?.name || ""));
	if (!policy || !widget) {
		return;
	}
	...
	if (policy === "strip-card") {
		const statusCard = element.firstElementChild;
		if (statusCard?.style) {
	...
	for (const row of element.children || []) {
		...
		if (first && first.tagName !== "BUTTON" && row.querySelector?.("button")) {
...
function suppressLegacyStatusWidgets(node) {
	for (const widget of node?.widgets || []) {
		suppressLegacyStatusWidget(node, widget);
	}
}

function scheduleLegacyStatusSuppression(node) {
	suppressLegacyStatusWidgets(node);
	requestAnimationFrame(() => suppressLegacyStatusWidgets(node));
	setTimeout(() => suppressLegacyStatusWidgets(node), 40);
	setTimeout(() => suppressLegacyStatusWidgets(node), 160);
}
...
function patchAddDomWidget(node) {
	...
	node.addDOMWidget = function (...args) {
		...
		suppressLegacyStatusWidget(this, widget);
...
function buildNodeMetaFromConfig(config = {}) {
	const meta = {
		name: String(config.nodeClass || config.name || ""),
		displayName: localizeLabel(config.displayName || config.display_name || config.nodeClass || "", ""),
		category: String(config.category || "GJJ"),
		description: String(config.description || "").trim(),
		help: {
			notice: Array.isArray(config.helpEntries) ? config.helpEntries.join("\n") : String(config.notice || ""),
	...
	parseInputMeta(inputSpec.required, meta);
	parseInputMeta(inputSpec.optional, meta);
	parseInputMeta(inputSpec.hidden, meta);
	const outputSpec = config.outputSpec || {};
	const outputNames = Array.isArray(outputSpec.outputNames) ? outputSpec.outputNames : [];
	const outputTooltips = Array.isArray(outputSpec.outputTooltips) ? outputSpec.outputTooltips : [];
	for (let index = 0; index < outputNames.length; index += 1) {
		meta.outputs.push({
			label: localizeLabel(outputNames[index], ""),
			tooltip: localizeTooltip(outputTooltips[index], "", outputNames[index]),
...
export function GJJ_STANDARDIZE_NODE(config = {}) {
	const className = String(config.nodeClass || config.name || "").trim();
	if (!className) {
		return;
	}
	META_BY_CLASS.set(className, buildNodeMetaFromConfig(config));
	if (config.enableStatus === true) {
		STATUS_ENABLED_CLASSES.add(className);
	}
	app.registerExtension({
		name: `GJJ.CommonNodeStandardizer.${className}`,
		beforeRegisterNodeDef(nodeType, nodeData) {
			if (String(nodeData?.name || "") !== className) {
				return;
			}
		...
		nodeCreated(node) {
			if (String(node?.comfyClass || node?.type || "") === className) {
				patchNode(node);
...
function ensureHelpStyles() {
	...
	style.textContent = `
		...
		.gjj-help-model-download {
			display: block;
			padding: 9px 10px;
			border: 1px solid rgba(76, 168, 137, 0.38);
			border-radius: 8px;
			background: rgba(35, 92, 72, 0.24);
			color: #a8f0cf;
			font-size: 13px;
			font-weight: 650;
			line-height: 1.45;
		...
		.gjj-help-model-download:hover {
			border-color: rgba(102, 213, 170, 0.68);
			background: rgba(39, 117, 88, 0.36);
			color: #d5ffe9;
		}
	`;
...
function forwardLegacyErrorStatus(detail = {}) {
	const targetId = String(detail?.node || "");
	const text = String(
		detail?.panel_message
	...
	for (const node of app.graph?._nodes || []) {
		if (!targetId || String(node.id) !== targetId || !shouldAttachStatus(node)) {
			continue;
		}
		patchNode(node);
		updateStatus(node, {
			text: text.startsWith("执行失败") ? text : `执行失败：${text}`,
			progress: 1,
...
function registerStatusClass(className) {
	const name = String(className || "").trim();
	if (!name) {
		return false;
	}
	STATUS_ENABLED_CLASSES.add(name);
	for (const node of app.graph?._nodes || []) {
		if (String(node?.comfyClass || node?.type || "") === name) {
			patchNode(node);
			ensureStatusWidget(node);
		}
	}
...
function unregisterStatusClass(className) {
	const name = String(className || "").trim();
	if (!name) {
		return false;
	}
	STATUS_ENABLED_CLASSES.delete(name);
	for (const node of app.graph?._nodes || []) {
		if (String(node?.comfyClass || node?.type || "") === name) {
			disableStatusWidget(node);
		}
...
globalThis.GJJ_CommonNodeStandardizer = {
	STATUS_WIDGET_NAME,
	HELP_WIDGET_NAME,
	isGjjNode,
	patchNode,
	...
	hasStatusClass(className) {
		return STATUS_ENABLED_CLASSES.has(String(className || ""));
	},
	registerStatusClass,
	unregisterStatusClass,
	listStatusClasses() {
		return Array.from(STATUS_ENABLED_CLASSES);
	},
};
...
app.registerExtension({
	...
	setup() {
		...
		// 依赖/模型缺失提示由 gjj_common_dependency_model_notice.js 专职处理；
		// 公共状态栏只负责执行状态、进度和完成信息。

		for (const eventName of LEGACY_ERROR_EVENTS) {
			api.addEventListener(eventName, (event) => {
				forwardLegacyErrorStatus(event?.detail || {});
			});
		}

...
function getFormulaElement(node) {
	const widget = GJJ_Utils.getWidget(node, FORMULA_WIDGET);
...
function getFormula(node) {
	return String(GJJ_Utils.getWidget(node, FORMULA_WIDGET)?.value || "");
}

function setFormula(node, formula, selectionStart = null, selectionEnd = null, validateNow = false) {
	setWidgetValue(GJJ_Utils.getWidget(node, FORMULA_WIDGET), formula);

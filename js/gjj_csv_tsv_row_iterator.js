...
function migrateOldWidgetValues(node, serialized) {
	...
	const timeout = findWidget(node, TIMEOUT_WIDGET);
...
function updateCurrentRowBounds(node, totalRows) {
	const widget = findWidget(node, CURRENT_ROW_WIDGET);
	if (!widget) {
		return;
	}
...
function createFilePicker(node) {
	...
	input.addEventListener("change", async () => {
		...
		const sourceWidget = findWidget(node, SOURCE_PATH_WIDGET);
		if (sourceWidget) {
...
function orderWidgets(node) {
	moveWidgetAfter(node, findWidget(node, STATE_WIDGET), TIMEOUT_WIDGET);

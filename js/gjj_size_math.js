...
function getAlignMultiple(node, fallback = DEFAULT_ALIGN_MULTIPLE) {
	return normalizePositiveInt(getWidget(node, ALIGN_WIDGET)?.value ?? fallback, 1);
}
...
function currentSize(node) {
	return {
		width: normalizePositiveInt(getWidget(node, WIDTH_WIDGET)?.value, 1),
		height: normalizePositiveInt(getWidget(node, HEIGHT_WIDGET)?.value, 1),
	};
}
...
function computeResolvedSize(node, baseWidth, baseHeight) {
	const mode = String(getWidget(node, MODE_WIDGET)?.value || "直接指定");
	const edgeLength = getWidget(node, EDGE_WIDGET)?.value;
	const rotationMode = getWidget(node, ROTATION_WIDGET)?.value;
...
function applySize(node, width, height) {
	setWidgetValue(getWidget(node, WIDTH_WIDGET), roundToNodeMultiple(node, width));
	setWidgetValue(getWidget(node, HEIGHT_WIDGET), roundToNodeMultiple(node, height));
}
...
function applyShortEdgePreset(node, shortEdge) {
	...
	node.__gjjSizeMathSyncing = true;
	try {
		setWidgetValue(getWidget(node, MODE_WIDGET), "直接指定");
		setWidgetValue(getWidget(node, EDGE_WIDGET), roundToNodeMultiple(node, shortEdge));
...
async function trySyncPrimaryImageSize(node) {
	...
	try {
		...
		setWidgetValue(getWidget(node, EDGE_WIDGET), normalizePositiveInt(Math.min(size.width, size.height), 1));
		const mode = String(getWidget(node, MODE_WIDGET)?.value || "直接指定");
...
function patchWidgetCallbacks(node) {
	...
	const ratioWidget = getWidget(node, ASPECT_RATIO_WIDGET);
	const widthWidget = getWidget(node, WIDTH_WIDGET);
	const heightWidget = getWidget(node, HEIGHT_WIDGET);
	const modeWidget = getWidget(node, MODE_WIDGET);
	const edgeWidget = getWidget(node, EDGE_WIDGET);
	const rotationWidget = getWidget(node, ROTATION_WIDGET);
	const alignWidget = getWidget(node, ALIGN_WIDGET);
		...
		widget.callback = function (value, ...args) {
			...
			if (!node.__gjjSizeMathSyncing) {
				if (!getPrimaryLinkedImageInput(node)?.link) {
					node.__gjjSizeMathBaseWidth = Number.parseInt(getWidget(node, WIDTH_WIDGET)?.value, 10) || 0;
					node.__gjjSizeMathBaseHeight = Number.parseInt(getWidget(node, HEIGHT_WIDGET)?.value, 10) || 0;
				}
				const ratio = guessRatioFromSize(
					node,
					getWidget(node, WIDTH_WIDGET)?.value,
					getWidget(node, HEIGHT_WIDGET)?.value,
				);
				if (ratio) {
					node.__gjjSizeMathSyncing = true;
					try {
	...
	node.onExecuted = function (message) {
		...
		if (message?.resolved_width?.[0] != null && message?.resolved_height?.[0] != null) {
			...
			try {
				...
				setWidgetValue(getWidget(node, WIDTH_WIDGET), message.resolved_width[0]);
				setWidgetValue(getWidget(node, HEIGHT_WIDGET), message.resolved_height[0]);
				if (message.short_edge?.[0] != null) {
					setWidgetValue(getWidget(node, EDGE_WIDGET), normalizePositiveInt(message.short_edge[0], 1));
				}
				if (message.aspect_ratio?.[0]) {
					setWidgetValue(getWidget(node, ASPECT_RATIO_WIDGET), message.aspect_ratio[0]);
				}
			} finally {
				node.__gjjSizeMathSyncing = false;
			}
...
function ensurePresetToolbar(node) {
	...
	for (const preset of MODEL_PRESETS) {
		modelRow.appendChild(createButton(preset.label, preset.title, () => {
			node.__gjjSizeMathSyncing = true;
			try {
				setWidgetValue(getWidget(node, ASPECT_RATIO_WIDGET), preset.ratio);
				setWidgetValue(getWidget(node, MODE_WIDGET), "直接指定");
				setWidgetValue(getWidget(node, EDGE_WIDGET), normalizePositiveInt(Math.min(preset.width, preset.height), 1));
				setWidgetValue(getWidget(node, OUTPUT_MODE_WIDGET), "当前尺寸");

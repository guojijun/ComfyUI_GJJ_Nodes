...
function panelSizeFromWidgets(node) {
	const width = persistedDimension(GJJ_Utils.getWidget(node, WIDTH_WIDGET)?.value);
	const height = persistedDimension(GJJ_Utils.getWidget(node, HEIGHT_WIDGET)?.value);
...
function applyPanelSize(node, width, height, options = {}) {
	...
	try {
		setWidgetValue(GJJ_Utils.getWidget(node, WIDTH_WIDGET), normalizeDimension(width));
		setWidgetValue(GJJ_Utils.getWidget(node, HEIGHT_WIDGET), normalizeDimension(height));
	} finally {
...
function multiLoaderSelection(sourceNode) {
	...
	const rawSelection = GJJ_Utils.getWidget(sourceNode, "selected_images")?.value || sourceNode?.properties?.selected_images || "[]";
...
function imageSourceSignature(node) {
	...
	const imageValue = GJJ_Utils.getWidget(sourceNode, "image")?.value;
	if (imageValue != null) {
...
function patchTransitionToggleCallback(node) {
	const widget = GJJ_Utils.getWidget(node, TRANSITION_TOGGLE_WIDGET);
...
function updateTransitionWidgets(node) {
	...
	const toggle = GJJ_Utils.getWidget(node, TRANSITION_TOGGLE_WIDGET);
...
function getStabilizeSignature(node) {
	return [
		...
		`transition:${widgetBooleanValue(GJJ_Utils.getWidget(node, TRANSITION_TOGGLE_WIDGET)) ? 1 : 0}`,
	].join("||");
}

...
function ensureStatusWidget(node) {
	if (node.__gjjAudioAceStatus) {
		return node.__gjjAudioAceStatus;
	}
	...
	node.__gjjAudioAceStatus = { widget, box };
	return node.__gjjAudioAceStatus;
}

function setStatus(node, text) {
	const box = node?.__gjjAudioAceStatus?.box;
	if (!box) {
		return;
	}
...
function patchNode(node) {
	if (!node || node.__gjjAudioAcePatched) {
		return;
	}
	node.__gjjAudioAcePatched = true;

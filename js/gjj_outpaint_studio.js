...
const SAMPLERS = ["euler", "euler_ancestral", "heun", "heunpp2", "dpm_2", "dpm_2_ancestral", "lms", "dpm_fast", "dpm_adaptive", "dpmpp_2s_ancestral", "dpmpp_sde", "dpmpp_sde_gpu", "dpmpp_2m", "dpmpp_3m", "ddim", "plms", "uni_pc", "uni_pc_bh2"];
const SCHEDULERS = ["normal", "karras", "exponential", "sgm_uniform", "simple", "ddim_uniform"];
const UPSCALE_METHODS = ["lanczos", "bilinear", "nearest", "bicubic"];
const SCALE_MODES = ["by_width", "by_height"];
const DIRECTIONS = ["all", "left+right", "top+bottom", "left", "right", "top", "bottom"];
...
function updateImagePreview(node, imageData) {
    ...
    const previewWidget = node.widgets?.find(w => w.name === "gjj_outpaint_preview");
    ...
    if (!url) {
        console.log("[GJJ] 无法转换为 URL:", imageData);
        image.style.display = "none";
        return;
    }

    console.log("[GJJ] 图片预览 URL:", url);
    image.src = url;
...
function hideDefaultPreviewElements(node) {
    ...
    if (nodeElement) {
        ...
        const previewClassNames = ["preview", "image", "output", "img", "thumbnail"];
        const previewIds = ["preview", "image", "output"];

        for (const el of allElements) {
            const classStr = String(el.className || "").toLowerCase();
            const idStr = String(el.id || "").toLowerCase();

            const hasPreviewClass = previewClassNames.some(cls => classStr.includes(cls));
            const hasPreviewId = previewIds.some(id => idStr.includes(id));

            if ((hasPreviewClass || hasPreviewId) && !idStr.includes("gjj")) {
    ...
    if (node.widgets) {
        for (const widget of node.widgets) {
            if (widget?.element && widget.name !== "gjj_outpaint_main" &&
                widget.name !== "gjj_outpaint_preview" && widget.name !== "gjj_outpaint_generate") {
                const img = widget.element.querySelector?.("img");
                if (img && !img.dataset?.gjjCustomPreview) {
                    img.style.display = "none";
                    img.style.visibility = "hidden";
                    img.style.height = "0px";
                }

...
const TEMPLATE_SOURCE_FIELDS = [
	...
	{ name: "visual_style", label: "整个视频", type: "STRING", aliases: ["visual_style", "style", "视觉风格", "整个视频"] },
...
function popup(node, key, title) {
	...
	for (const [sectionTitle, names] of POPUP_GROUPS[key] || []) {
		...
		const grid = document.createElement("div"); grid.className = "gjj-mh3-grid"; if (key === "promptBook" && sectionTitle === "提示词结构") grid.classList.add("gjj-mh3-inline-grid");

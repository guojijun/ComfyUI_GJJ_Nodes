...
// Boolean 控件名称
const BOOL_WIDGETS = ["auto_transcribe", "text_frontend"];

// ─── 只刷新当前节点 ──
function isExecutionOutputNode(node) {
	if (!node) return false;
	if (node === undefined || node === null) return false;
	if (node.comfyClass === "GJJ_CosyVoice3Generator") return true;
...
function ensureStatusWidget(node) {
	...
	const boolButtons = {};
	const boolConfigs = [
		{ name: "auto_transcribe", label: "📝 自动转录", default: true },
		{ name: "text_frontend", label: "🔤 文本前端", default: true },
	];
...
api.addEventListener("gjj_cosyvoice3_error", (event) => {
	const detail = event?.detail || {};
	const targetNode = app.graph?._nodes?.find((node) => String(node?.id) === String(detail.node));
	if (!targetNode || !TARGET_NODES.has(String(targetNode.comfyClass || targetNode.type || ""))) {
		return;

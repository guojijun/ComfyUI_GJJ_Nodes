...
function sourceWidgetValue(sourceNode) {
	...
	for (const name of names) {
		const widget = GJJ_Utils.getWidget(sourceNode, name);
...
async function loadSizeFromSource(sourceNode, link) {
	...
	const imageName = String(GJJ_Utils.getWidget(sourceNode, "image")?.value || "").trim();
	if (imageName) {
	...
	const videoWidget = GJJ_Utils.getWidget(sourceNode, "video") || GJJ_Utils.getWidget(sourceNode, "video_path") || GJJ_Utils.getWidget(sourceNode, "filename") || GJJ_Utils.getWidget(sourceNode, "path");
...
function applySize(node, size, signature) {
	...
	setWidgetValue(GJJ_Utils.getWidget(node, WIDTH_WIDGET), size.width);
	setWidgetValue(GJJ_Utils.getWidget(node, HEIGHT_WIDGET), size.height);

...
function refreshNode(node) {
	...
	const compactHeight = computeCompactHeight(node, computed?.[1] || node?.size?.[1] || 0);
	node.setSize?.([computed?.[0] || node?.size?.[0], compactHeight]);
	node.setDirtyCanvas?.(true, true);
	app.graph?.setDirtyCanvas?.(true, true);
}
...
function ensureLayoutWidget(node) {
	return getWidget(node, LAYOUT_WIDGET_NAME);
}
...
function updateModePanel(node) {
	...
	for (const [name, control] of state.controls.entries()) {
		syncCustomControlFromWidget(control, getWidget(node, name));
	}

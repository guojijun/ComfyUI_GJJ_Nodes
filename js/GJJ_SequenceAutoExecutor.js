...
function setStatus(node, text) {
	...
	let widget = GJJ_Utils.getWidget(node, STATUS_WIDGET);
	if (!widget) {
...
function ensureControlButtons(node) {
	...
	stopButton.addEventListener("click", (event) => {
		event.preventDefault();
		event.stopPropagation();
		const enabled = GJJ_Utils.getWidget(node, ENABLED_WIDGET);
		if (enabled) {
			enabled.value = false;
		}
		autoQueueCount = 0;
...
function enabledValue(node) {
	return Boolean(GJJ_Utils.getWidget(node, ENABLED_WIDGET)?.value);
}
...
function resetSequence(node) {
	...
	if (upstream?.comfyClass === INCREMENTING_NODE) {
		const valueWidget = GJJ_Utils.getWidget(upstream, INCREMENTING_VALUE_WIDGET);
	...
	const currentWidget = GJJ_Utils.getWidget(node, CURRENT_VALUE_WIDGET);
...
function upstreamStepCount(node) {
	...
	const value = Number(GJJ_Utils.getWidget(upstream, INCREMENTING_COUNT_WIDGET)?.value ?? 1);

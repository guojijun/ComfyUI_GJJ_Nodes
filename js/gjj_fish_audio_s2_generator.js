...
const TARGET_NODES = new Set(["GJJ_FishAudioS2Generator"]);
const AUDIO_PREFIX = "speaker_";
const MAX_SPEAKERS = 10;
const MIN_VISIBLE_PAIRS = 1;
...
function ensureStatusWidget(node) {
	if (node.__gjjFishAudioS2Status) {
	...
	GJJ_Utils.applyDependencyCopyButton(copyBtn, { visible: false, compact: true });
...
function progressFromText(text) {
	...
	if (value.includes("保存")) return 96;
	if (value.includes("拼接")) return 90;
	if (value.includes("合成")) return 58;
	if (value.includes("编码")) return 24;
	if (value.includes("准备")) return 18;
	if (value.includes("加载")) return 8;
...
function ensureAudioWidget(node) {
	if (node.__gjjFishAudioS2Audio) {
		return node.__gjjFishAudioS2Audio;
	}
	const box = document.createElement("div");
	box.style.cssText = [
		"display:none",
	...
	node.__gjjFishAudioS2Audio = { widget, box, audio, openLink, downloadLink };
	return node.__gjjFishAudioS2Audio;
}
...
function setAudioPreview(node, message) {
	...
	audioWidget.downloadLink.download = item.filename || "GJJ_FishAudioS2.mp3";
...
function renamePairsSequentially(node) {
	...
	for (const [zeroIndex, pair] of pairs.entries()) {
		...
		if (pair.text) {
			...
			pair.text.tooltip = `第 ${index} 个参考音频对应的文字，可留空。`;
		}
	}
}
...
function stabilizeNode(node) {
	...
	node.__gjjFishAudioS2LinkSignature = getLinkSignature(node);
	refreshNode(node);
}

function scheduleStabilize(node, ms = 32) {
	clearTimeout(node.__gjjFishAudioS2StabilizeTimer);
	node.__gjjFishAudioS2StabilizeTimer = setTimeout(() => stabilizeNode(node), ms);
}
...
api.addEventListener("gjj_fish_audio_s2_error", (event) => {
	const detail = event?.detail || {};
	const targetNode = app.graph?._nodes?.find((node) => String(node?.id) === String(detail.node));
	if (!targetNode || !TARGET_NODES.has(String(targetNode.comfyClass || targetNode.type || ""))) {
		return;
	...
	patchNode(targetNode);
	const status = targetNode.__gjjFishAudioS2Status;
	const errorMessage = detail.error || "";

	if (status?.label) {
		status.label.textContent = `❌ ${errorMessage || "执行失败"}`;
	}
	if (status?.copyBtn) {
...
app.registerExtension({
	name: "Comfy.GJJ.FishAudioS2Generator",
	...
	beforeRegisterNodeDef(nodeType, nodeData) {
		...
		nodeType.prototype.onDrawBackground = function (...args) {
			...
			if (signature !== this.__gjjFishAudioS2LinkSignature) {

...
def download(sub_dir, name, force=False, root='~/.insightface'):
    ...
    # GJJ keeps insightface models under ComfyUI/models/insightface directly.
    # Upstream insightface expects root/models/<name>; support both layouts and
    # never download when a local model directory already exists.
    direct_dir_path = os.path.join(_root, name)
    if osp.exists(direct_dir_path) and not force:
        return direct_dir_path

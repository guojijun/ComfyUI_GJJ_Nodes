...
def get_providers_from_kwargs(kwargs):
...
def resolve_model_file(name, root):
    if name.endswith('.onnx'):
        return name

    # GJJ compatibility: some callers pass root=ComfyUI/models/insightface,
    # while upstream insightface expects root/models/<name>.
    for model_dir in (osp.join(root, name), osp.join(root, 'models', name)):
...
def get_model(name, **kwargs):
    ...
    model_file = resolve_model_file(name, root)
    ...
    providers = get_providers_from_kwargs(kwargs)

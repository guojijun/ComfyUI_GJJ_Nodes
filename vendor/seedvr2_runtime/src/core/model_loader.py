...
replace_linear_with_quantized = None
traceback = None


def _ensure_gguf_runtime(operation: str = "load GGUF model", debug: Optional['Debug'] = None):
    """
    Lazily import GGUF-only dependencies. Non-GGUF models must not require the
    gguf package to be installed.
    """
    global gguf, dequantize_tensor, replace_linear_with_quantized, traceback
    if gguf is not None and dequantize_tensor is not None and replace_linear_with_quantized is not None:
        return gguf
    try:
        gguf_module = importlib.import_module("gguf")
        compat = importlib.import_module("..optimization.compatibility", __package__)
        compat.gguf = gguf_module
        compat.GGMLQuantizationType = gguf_module.GGMLQuantizationType
        compat.GGUF_AVAILABLE = True
        gguf_dequant = importlib.import_module("..optimization.gguf_dequant", __package__)
        gguf_dequant.gguf = gguf_module
        gguf_dequant.GGUF_AVAILABLE = True
        gguf_dequant.TORCH_COMPATIBLE_QTYPES = (
            None,
            gguf_module.GGMLQuantizationType.F32,
            gguf_module.GGMLQuantizationType.F16,
        )
        gguf_dequant.dequantize_functions = {
            gguf_module.GGMLQuantizationType.BF16: gguf_dequant.dequantize_blocks_BF16,
            gguf_module.GGMLQuantizationType.Q8_0: gguf_dequant.dequantize_blocks_Q8_0,
            gguf_module.GGMLQuantizationType.Q5_1: gguf_dequant.dequantize_blocks_Q5_1,
            gguf_module.GGMLQuantizationType.Q5_0: gguf_dequant.dequantize_blocks_Q5_0,
            gguf_module.GGMLQuantizationType.Q4_1: gguf_dequant.dequantize_blocks_Q4_1,
            gguf_module.GGMLQuantizationType.Q4_0: gguf_dequant.dequantize_blocks_Q4_0,
            gguf_module.GGMLQuantizationType.Q6_K: gguf_dequant.dequantize_blocks_Q6_K,
            gguf_module.GGMLQuantizationType.Q5_K: gguf_dequant.dequantize_blocks_Q5_K,
            gguf_module.GGMLQuantizationType.Q4_K: gguf_dequant.dequantize_blocks_Q4_K,
            gguf_module.GGMLQuantizationType.Q3_K: gguf_dequant.dequantize_blocks_Q3_K,
            gguf_module.GGMLQuantizationType.Q2_K: gguf_dequant.dequantize_blocks_Q2_K,
        }
        gguf_ops = importlib.import_module("..optimization.gguf_ops", __package__)
        gguf_ops.gguf = gguf_module
        gguf_ops.GGUF_AVAILABLE = True
        gguf_ops.is_gguf_quantized = gguf_dequant.is_quantized
        gguf_ops.dequantize_tensor = gguf_dequant.dequantize_tensor
        gguf = gguf_module
        dequantize_tensor = gguf_dequant.dequantize_tensor
        replace_linear_with_quantized = gguf_ops.replace_linear_with_quantized
        traceback = importlib.import_module("traceback")
        return gguf
    except Exception as exc:
        error_msg = (
            f"Cannot {operation}: GGUF library is not installed or failed to load.\n"
            "Install gguf, or choose a non-GGUF model such as .safetensors."
        )
        if debug:
            debug.log(error_msg, level="ERROR", category="setup", force=True)
        raise RuntimeError(f"GGUF library required to {operation}") from exc


def _has_comfy_quant_tensors(state: Dict[str, torch.Tensor]) -> bool:
    return any(name.endswith(".comfy_quant") for name in state)


def _replace_linear_with_comfy_quant_linear(
    ...
    """
    Replace Linear modules that have Comfy .comfy_quant metadata with Comfy's
    quant-aware Linear so load_state_dict can bind packed ConvRot/NVFP4/etc.
    """
    try:
        import comfy.ops as comfy_ops
    except Exception as exc:
        raise RuntimeError(
            "This safetensors file contains Comfy quantized weights (.comfy_quant), "
            "but comfy.ops could not be imported. Use a normal fp16/fp8/GGUF model "
            "or run under a ComfyUI build that supports Comfy quantized tensors."
        ) from exc
    ...
    for child_name, child in list(model.named_children()):
        ...
        if isinstance(child, torch.nn.Linear) and f"{child_prefix}comfy_quant" in state:
            replacement = mixed_ops.Linear(
                child.in_features,
                child.out_features,
            ...
            replaced += _replace_linear_with_comfy_quant_linear(
                child,
                state,
                target_device,
                compute_dtype,
...
def load_quantized_state_dict(checkpoint_path: str, device: torch.device = torch.device("cpu"),
    ...
    if checkpoint_path.endswith('.safetensors'):
        ...
        _ensure_gguf_runtime(f"load {os.path.basename(checkpoint_path)}", debug)
...
def _load_gguf_state(checkpoint_path: str, device: torch.device, debug: Optional['Debug'] = None,
    ...
    """
    gguf_module = _ensure_gguf_runtime(f"load {os.path.basename(checkpoint_path)}", debug)
    reader = gguf_module.GGUFReader(checkpoint_path)
    ...
    for i, (sd_key, tensor) in enumerate(tensors):
        ...
        if tensor.tensor_type in {gguf_module.GGMLQuantizationType.F32, gguf_module.GGMLQuantizationType.F16}:
...
def _get_tensor_logical_shape(reader: 'gguf.GGUFReader', tensor_name: str) -> Optional[torch.Size]:
    ...
    gguf_module = _ensure_gguf_runtime("read GGUF tensor metadata")
    if len(field.types) != 2 or field.types[0] != gguf_module.GGUFValueType.ARRAY or field.types[1] != gguf_module.GGUFValueType.INT32:
...
class GGUFTensor(torch.Tensor):
    ...
    def dequantize(self, device=None, dtype=torch.float16, dequant_dtype=None):
        ...
        gguf_module = _ensure_gguf_runtime("dequantize GGUF tensor", getattr(self, "debug", None))
        if self.tensor_type in {gguf_module.GGMLQuantizationType.F32, gguf_module.GGMLQuantizationType.F16}:
        ...
        try:
            _ensure_gguf_runtime("dequantize GGUF tensor", getattr(self, "debug", None))
        ...
        try:
            ...
            gguf_module = _ensure_gguf_runtime("dequantize GGUF tensor", getattr(self, "debug", None))
            dequantized = gguf_module.quants.dequantize(numpy_data, self.tensor_type)
            ...
            traceback_module = importlib.import_module("traceback")
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        if getattr(func, "__name__", "") in {"numel", "nelement", "element_size"}:
            return super().__torch_function__(func, types, args, kwargs)

        # Check if tensor is already unquantized (F32/F16)
        gguf_module = _ensure_gguf_runtime("dequantize GGUF tensor", debug)
        if tensor_type in {gguf_module.GGMLQuantizationType.F32, gguf_module.GGMLQuantizationType.F16}:
            return super().__torch_function__(func, types, args, kwargs)
...
def _load_standard_weights(model: torch.nn.Module, state: Dict[str, torch.Tensor], 
                          ...
                          compute_dtype: Optional[torch.dtype] = None) -> torch.nn.Module:
    """Load standard (non-GGUF) weights into model."""
    debug.start_timer(f"{model_type_lower}_state_apply")
    if _has_comfy_quant_tensors(state):
        for key, value in list(state.items()):
            if key.endswith(".comfy_quant") and torch.is_tensor(value) and value.device.type != "cpu":
                state[key] = value.cpu()
        ...
        if debug:
            debug.log(
                f"Detected Comfy quantized safetensors; replaced {replaced} Linear layers for native quant loading",
...
def _load_gguf_weights(model: torch.nn.Module, state: Dict[str, torch.Tensor], 
    ...
    if stats['quantized'] > 0:
        ...
        _ensure_gguf_runtime("replace GGUF quantized layers", debug)
...
def _create_dequantize_method(tensor: torch.Tensor, debug: Optional['Debug'] = None) -> callable:
    ...
    def dequantize(device: Optional[torch.device] = None, 
        ...
        try:
            ...
            dequantized = gguf_module.quants.dequantize(numpy_data, tensor.tensor_type)

...
ENABLE_OPTIONAL_ATTN = os.environ.get("GJJ_FLASHVSR_ENABLE_OPTIONAL_ATTN", "").strip().lower() in {"1", "true", "yes", "on"}

if ENABLE_OPTIONAL_ATTN:
    try:
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False, attention_mask=None, return_KV=False):
    if attention_mask is not None:
        ...
        if USE_BLOCK_ATTN and BLOCK_ATTN_AVAILABLE:
        ...
        elif SPARSE_SAGE_ATTN_AVAILABLE:
            x = sparse_sageattn(
                q, k, v,
            ...
            dense_mask = base_blockmask.to(torch.bool)
            try:
                x = F.scaled_dot_product_attention(q, k, v, attn_mask=dense_mask)
            except Exception:

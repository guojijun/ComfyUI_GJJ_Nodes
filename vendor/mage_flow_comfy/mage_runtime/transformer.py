...
from .attention_backend import flash_attn_varlen_func
...
class RMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, elementwise_affine=True, bias=False, dtype=None, device=None):
        super().__init__()
        self.normalized_shape = (hidden_size,)
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(hidden_size, dtype=dtype, device=device)) if elementwise_affine else None
        self.bias = nn.Parameter(torch.zeros(hidden_size, dtype=dtype, device=device)) if bias else None

    def forward(self, x):
        weight = self.weight
        if weight is not None and weight.dtype != x.dtype:
            weight = weight.to(dtype=x.dtype, device=x.device)
        y = F.rms_norm(x, self.normalized_shape, weight, self.eps)
...
class GELUApproximate(nn.Module):
    def __init__(self, dim, inner_dim, dtype=None, device=None, operations=None):
        super().__init__()
        ops = operations or TorchOps
        self.proj = ops.Linear(dim, inner_dim, bias=True, dtype=dtype, device=device)
...
class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, dtype=None, device=None, operations=None):
        super().__init__()
        ...
        self.net = nn.ModuleList(
            [
                GELUApproximate(dim, inner_dim, dtype=dtype, device=device, operations=ops),
                nn.Dropout(0.0),
                ops.Linear(inner_dim, dim_out, bias=True, dtype=dtype, device=device),
            ]
        )
...
class TimestepEmbedding(nn.Module):
    def __init__(self, in_channels, time_embed_dim, dtype=None, device=None, operations=None):
        super().__init__()
        ops = operations or TorchOps
        self.linear_1 = ops.Linear(in_channels, time_embed_dim, bias=True, dtype=dtype, device=device)
        self.act = nn.SiLU()
        self.linear_2 = ops.Linear(time_embed_dim, time_embed_dim, bias=True, dtype=dtype, device=device)
...
class Attention(nn.Module):
    def __init__(
        ...
        self.norm_q = RMSNorm(dim_head, eps=eps, elementwise_affine=elementwise_affine, dtype=dtype, device=device)
        self.norm_k = RMSNorm(dim_head, eps=eps, elementwise_affine=elementwise_affine, dtype=dtype, device=device)

        self.to_q = ops.Linear(query_dim, self.inner_dim, bias=bias, dtype=dtype, device=device)
        self.to_k = ops.Linear(self.cross_attention_dim, self.inner_kv_dim, bias=bias, dtype=dtype, device=device)
        self.to_v = ops.Linear(self.cross_attention_dim, self.inner_kv_dim, bias=bias, dtype=dtype, device=device)

        self.added_proj_bias = added_proj_bias
        if self.added_kv_proj_dim is not None:
            self.add_k_proj = ops.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias, dtype=dtype, device=device)
            self.add_v_proj = ops.Linear(added_kv_proj_dim, self.inner_kv_dim, bias=added_proj_bias, dtype=dtype, device=device)
            self.add_q_proj = ops.Linear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias, dtype=dtype, device=device)
            self.norm_added_q = RMSNorm(dim_head, eps=eps, dtype=dtype, device=device)
            self.norm_added_k = RMSNorm(dim_head, eps=eps, dtype=dtype, device=device)
        else:
        ...
        self.to_out.append(ops.Linear(self.inner_dim, self.out_dim, bias=out_bias, dtype=dtype, device=device))
        self.to_out.append(nn.Dropout(dropout))

        self.to_add_out = ops.Linear(self.inner_dim, self.out_context_dim, bias=out_bias, dtype=dtype, device=device)
...
class MageFlowTransformerBlock(nn.Module):
    def __init__(
        ...
        self.img_mod = nn.Sequential(
            nn.SiLU(),
            ops.Linear(dim, 6 * dim, bias=True, dtype=dtype, device=device),  # For scale, shift, gate for norm1 and norm2
        )
        self.img_norm1 = ops.LayerNorm(dim, elementwise_affine=False, eps=eps, dtype=dtype, device=device)
        self.attn = Attention(
            query_dim=dim,
        ...
        self.img_norm2 = ops.LayerNorm(dim, elementwise_affine=False, eps=eps, dtype=dtype, device=device)
        self.img_mlp = FeedForward(dim=dim, dim_out=dim, dtype=dtype, device=device, operations=ops)
        ...
        self.txt_norm1 = ops.LayerNorm(dim, elementwise_affine=False, eps=eps, dtype=dtype, device=device)
        # Text doesn't need separate attention - it's handled by img_attn joint computation
        self.txt_norm2 = ops.LayerNorm(dim, elementwise_affine=False, eps=eps, dtype=dtype, device=device)
        self.txt_mlp = FeedForward(dim=dim, dim_out=dim, dtype=dtype, device=device, operations=ops)
...
class AdaLayerNormContinuous(nn.Module):
    ...
    def __init__(
        ...
        ops = operations or TorchOps
        self.silu = nn.SiLU()
        self.linear = ops.Linear(conditioning_embedding_dim, embedding_dim * 2, bias=bias, dtype=dtype, device=device)
        if norm_type == "layer_norm":
            self.norm = ops.LayerNorm(embedding_dim, eps, elementwise_affine, bias, dtype=dtype, device=device)
        elif norm_type == "rms_norm":
            self.norm = RMSNorm(embedding_dim, eps, elementwise_affine, dtype=dtype, device=device)
        else:

...
class MageVAE(nn.Module):
    ...
    def __init__(self, ckpt_path: str | None = None, sample_posterior: bool = True, state_dict: dict | None = None):
        super().__init__()
        ...
        if state_dict is None:
            if ckpt_path is None:
                raise ValueError("MageVAE requires either ckpt_path or state_dict.")
            state_dict = _load_state_dict(ckpt_path)
        source_name = ckpt_path or "<state_dict>"
        self._load_encoder(state_dict, source_name)
        self._load_decoder(state_dict, source_name)
    ...
    @property
    def device(self):
        comfy_device = getattr(self, "_comfy_device", None)
        if comfy_device is not None:
            return comfy_device
        return next(self.parameters()).device

    @device.setter
    def device(self, value):
        self._comfy_device = torch.device(value)

...
from .geometry_types import SPLAT
...
def _gjj_work_device():
    if os.environ.get("GJJ_TRIPOSPLAT_VRAM_FIRST") == "1":
...
def _real_len(g: SPLAT, i: int) -> int:
...
def _gaussian_item(g: SPLAT, i: int, device):
...
def _render_gaussian(xyz, rgb, opacity, scale, rot, width, height, splat_scale, bg, camera_info,
    ...
    idev, idtype = _gjj_work_device(), comfy.model_management.intermediate_dtype()
...
class TransformSplat(IO.ComfyNode):
    ...
    def execute(cls, splat, translate_x, translate_y, translate_z,
        ...
        out = SPLAT(positions, scales, rotations, splat.opacities, splat.sh,
...
def _merge_gaussians(gaussians: list) -> SPLAT:
    ...
    return SPLAT(_pad_stack(pos_b, n), _pad_stack(scl_b, n), _pad_stack(rot_b, n),
...
def _gaussian_to_mesh(g: SPLAT, i, res, kernel, taubin, level_bias, min_component, min_opacity, color_sharpen, device, progress=None):

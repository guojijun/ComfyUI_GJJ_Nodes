...
class GJJ_TextInput:
            ...
            "required": {
                "text": (
                    "STRING",
                    {
                        ...
                        "display": "hidden",
                        "display_name": "文本内容",
                        "tooltip": "此参数由前端 Markdown 编辑器维护并隐藏；未连接外部文本时会输出这里保存的内容。",
                    },
                ),
            },
            "optional": {
    ...
    def output_text(self, text: str, text_in: str | None = None):
        result = normalize_text(text_in) if text_in else normalize_text(text)
        return (result,)


NODE_CLASS_MAPPINGS = {NODE_NAME: GJJ_TextInput}
NODE_DISPLAY_NAME_MAPPINGS = {NODE_NAME: "GJJ · 📝 文本输入、预览(Markdown)"}

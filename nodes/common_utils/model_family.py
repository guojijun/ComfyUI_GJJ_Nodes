...
def gjjutils_pick_available_model_name(
    ...
    *,
    ...
    if preferred:
        scored = [
            (_candidate_score(preferred, name), index, name)
    ...
    if fallback and fallback != preferred:
        resolved = gjjutils_pick_available_model_name(
            fallback, names, "", allow_first=False
        )
        if resolved:
            return resolved
...
def gjjutils_load_model_family_presets() -> list[dict[str, Any]]:
    ...
    presets = [_parse_preset_row(row) for row in reader]

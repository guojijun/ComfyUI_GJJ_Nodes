...
def _batch_norm2d(num_features, device=None, dtype=None, operations=None):
    batch_norm = getattr(operations, "BatchNorm2d", None) if operations is not None else None
    if batch_norm is not None:
        return batch_norm(num_features, device=device, dtype=dtype)
    return nn.BatchNorm2d(num_features, device=device, dtype=dtype)
...
class BasicDecBlk(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, inter_channels=64, device=None, dtype=None, operations=None):
        ...
        self.bn_in = _batch_norm2d(inter_channels, device=device, dtype=dtype, operations=operations)
        self.bn_out = _batch_norm2d(out_channels, device=device, dtype=dtype, operations=operations)
...
class _ASPPModuleDeformable(nn.Module):
    def __init__(self, in_channels, planes, kernel_size, padding, device, dtype, operations):
        ...
        self.bn = _batch_norm2d(planes, device=device, dtype=dtype, operations=operations)
...
class ASPPDeformable(nn.Module):
    def __init__(self, in_channels, out_channels=None, parallel_block_sizes=[1, 3, 7], device=None, dtype=None, operations=None):
        ...
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             ...
                                             _batch_norm2d(self.in_channelster, device=device, dtype=dtype, operations=operations),
        ...
        self.bn1 = _batch_norm2d(out_channels, device=device, dtype=dtype, operations=operations)
        self.relu = nn.ReLU(inplace=True)
...
class Decoder(nn.Module):
    def __init__(self, channels, device, dtype, operations):
        ...
        self.gdt_convs_4 = nn.Sequential(operations.Conv2d(channels[0] // 2, _N, 3, 1, 1, **fk), _batch_norm2d(_N, **fk), nn.ReLU(inplace=True))
        self.gdt_convs_3 = nn.Sequential(operations.Conv2d(channels[1] // 2, _N, 3, 1, 1, **fk), _batch_norm2d(_N, **fk), nn.ReLU(inplace=True))
        self.gdt_convs_2 = nn.Sequential(operations.Conv2d(channels[2] // 2, _N, 3, 1, 1, **fk), _batch_norm2d(_N, **fk), nn.ReLU(inplace=True))

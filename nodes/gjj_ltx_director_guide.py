...
from .gjj_ltx_director import GuideData, MotionGuideData, _resize_image
...
class GJJLTXDirectorGuide:
    ...
    GJJ_HELP = {
        "title": "LTX 导演 Guide 引导",
        "version": "2.0.2",
        "description": DESCRIPTION,
        "features": [
            "把 LTX 导演时间线输出的图像 Guide 插入视频 Latent 对应帧。",
            "加载 IC-LoRA 并把时间线运动视频编码为 Motion Guide。",
            "支持 Retake 基础视频编码、时间遮罩和两阶段局部重做。",
            "支持 Latent 缩放、IC 网格自动对齐和分块 VAE 编码。",
        ],
        "usage": [
            "连接正负条件、视频 VAE、视频 Latent 和 Director 的 Guide 数据。",
            "使用运动引导时，再连接 Motion Guide 数据、模型并选择 IC-LoRA。",
            "显存不足时开启分块 VAE 编码，并根据显存调整分块尺寸和重叠。",
            "执行后将正负条件、Guide 视频 Latent 和模型继续连接到采样链。",
        ],
        "notes": [
            "Retake 模式通常由 Director 时间线自动识别，也可用强制 Retake 模式手动开启。",
            "IC-LoRA 模型元数据缺少 reference_downscale_factor 时会按 1.0 处理。",
            "完成采样后可使用 LTX 导演 Guide 裁剪节点移除附加 Guide Latent 帧。",
        ],
    }
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "positive": ("CONDITIONING", {"display_name": "正向条件", "tooltip": "需要写入 Guide 关键帧信息的正向条件。"}),
                "negative": ("CONDITIONING", {"display_name": "负向条件", "tooltip": "需要写入 Guide 关键帧信息的负向条件。"}),
                "vae": ("VAE", {"display_name": "视频VAE", "tooltip": "用于把图像和运动视频 Guide 编码到 LTX 视频 Latent 空间。"}),
                "latent": ("LATENT", {"display_name": "视频Latent", "tooltip": "要插入图像关键帧和运动 Guide 的视频 Latent。"}),
                "guide_data": ("GUIDE_DATA", {"display_name": "Guide数据", "tooltip": "来自 GJJ LTX 导演时间线的图像 Guide、插入帧和强度数据。"}),
            },
            "optional": {
                "motion_guide_data": ("MOTION_GUIDE_DATA", {"display_name": "Motion Guide数据", "tooltip": "连接 Director 的 Motion Guide 数据，启用 IC-LoRA 运动视频引导。"}),
                "model": ("MODEL", {"display_name": "模型", "tooltip": "使用 IC-LoRA 运动引导时连接；节点会把所选 LoRA 应用到该模型。"}),
                "ic_lora_name": (["None"] + loras, {"default": "None", "display_name": "IC-LoRA模型", "tooltip": "选择用于运动视频引导的 IC-LoRA；None 表示不加载。"}),
                "ic_lora_strength": ("FLOAT", {"default": 1.0, "min": -100.0, "max": 100.0, "step": 0.01, "display_name": "IC-LoRA强度", "tooltip": "IC-LoRA 应用到模型的强度；0 表示不改变模型权重。"}),
                "scale_by": ("FLOAT", {"default": 1.0, "min": 0.01, "max": 8.0, "step": 0.01, "display_name": "Latent缩放", "tooltip": "写入 Guide 前对视频 Latent 空间宽高进行缩放；1 表示不缩放。"}),
                "upscale_method": (["nearest-exact", "bilinear", "area", "bicubic", "bislerp"], {"default": "bicubic", "display_name": "缩放算法", "tooltip": "视频 Latent 放大或缩小时使用的插值算法。"}),
                "image_attention_strength": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.01, "display_name": "图像注意力强度", "tooltip": "IC-LoRA 启用时，图像 Guide 在 Guide Attention 中的权重。"}),
                "crop": (["disabled", "center"], {"default": "center", "display_name": "IC-LoRA裁剪", "tooltip": "运动视频 Guide 的裁剪方式；center 为居中裁剪，disabled 为拉伸适配。"}),
                "auto_snap_ic_grid": ("BOOLEAN", {"default": True, "display_name": "自动对齐IC网格", "tooltip": "根据 IC-LoRA 的 reference_downscale_factor 自动调整 Latent 网格尺寸。"}),
                "use_tiled_encode": ("BOOLEAN", {"default": False, "display_name": "分块VAE编码", "tooltip": "开启后分块编码 Guide 图像和视频，可降低峰值显存占用。"}),
                "tile_size": ("INT", {"default": 256, "min": 64, "max": 512, "step": 32, "display_name": "分块尺寸", "tooltip": "分块 VAE 编码使用的单块像素尺寸。"}),
                "tile_overlap": ("INT", {"default": 64, "min": 16, "max": 256, "step": 16, "display_name": "分块重叠", "tooltip": "相邻 VAE 编码分块的重叠像素，用于减轻接缝。"}),
                "retake_mode": ("BOOLEAN", {"default": False, "display_name": "强制Retake模式", "tooltip": "强制按 Retake 路径执行；关闭时仍会从 Director 时间线数据自动识别 Retake。"}),
            }
        }

    RETURN_TYPES = ("CONDITIONING", "CONDITIONING", "LATENT", "MODEL", "FLOAT")
    RETURN_NAMES = ("正向条件", "负向条件", "Guide视频Latent", "模型", "Latent降采样系数")
    OUTPUT_TOOLTIPS = (
        "已追加图像关键帧和运动 Guide 信息的正向条件。",
        "已追加图像关键帧和运动 Guide 信息的负向条件。",
        "已插入 Guide Latent 与噪声遮罩的视频 Latent。",
        "应用 IC-LoRA 后的模型；未连接模型时输出为空。",
        "从 IC-LoRA 元数据读取的 Latent 降采样系数。",
    )
    FUNCTION = "execute"
        ...
        if is_retake_active:
            ...
            if video_file and need_base_video:
                try:
                    ...
                    log.warning("[LTXDirectorGuide] Failed to load/encode base video: %s", e)
...
class GJJLTXDirectorCropGuides:
    ...
    GJJ_HELP = {
        "title": "LTX 导演 Guide 裁剪",
        "version": "2.0.2",
        "description": DESCRIPTION,
        "usage": [
            "把 Guide 引导节点输出的正负条件和采样后的 Latent 接入。",
            "节点会根据条件中记录的精确数量裁掉末尾附加 Guide Latent 帧。",
            "裁剪后的正负条件和 Latent 可继续用于 VAE 解码或后续处理。",
        ],
        "notes": [
            "没有检测到附加 Guide 帧时会原样透传。",
            "裁剪后会清除 keyframe_idxs、guide_attention_entries 和内部裁剪计数。",
        ],
    }
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "positive": ("CONDITIONING", {"display_name": "正向条件", "tooltip": "Guide 引导节点输出的正向条件，包含附加 Guide 帧计数。"}),
                "negative": ("CONDITIONING", {"display_name": "负向条件", "tooltip": "Guide 引导节点输出的负向条件。"}),
                "latent": ("LATENT", {"display_name": "采样Latent", "tooltip": "完成采样、仍包含附加 Guide Latent 帧的结果。"}),
            }
        }

    RETURN_TYPES = ("CONDITIONING", "CONDITIONING", "LATENT")
    RETURN_NAMES = ("正向条件", "负向条件", "裁剪Latent")
    OUTPUT_TOOLTIPS = (
        "已清除 Guide 关键帧元数据的正向条件。",
        "已清除 Guide 关键帧元数据的负向条件。",
        "已移除末尾附加 Guide 帧的 Latent。",
    )
...
NODE_CLASS_MAPPINGS = {
    ...
    "GJJ_LTXDirectorCropGuides": GJJLTXDirectorCropGuides,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "GJJ_LTXDirectorGuide": "🧭 LTX导演Guide引导",
    "GJJ_LTXDirectorCropGuides": "✂️ LTX导演Guide裁剪",
}

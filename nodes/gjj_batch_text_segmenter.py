...
_ENV_REPORT = build_sam3_environment_report(DEFAULT_SAM3_MODEL)
_ENV_READY = bool(_ENV_REPORT.get("available", True))
_DESCRIPTION_READY = "GJJ 零依赖批量 SAM3 文本分割器：输入 GJJ_BATCH_IMAGE 和分号分段文本，按图文序号或顺序匹配图片，调用本地 models/sam3 模型输出 RGBA 透明裁剪批量图。无联网、无第三方自定义节点依赖；无法识别时只在面板显示警告并跳过。"
...
class GJJ_BatchTextSegmenter:
	...
	GJJ_HELP = {
		"title": "GJJ · ✂️批量文本分割器抠图(SAM3)",
		"description": _ENV_REPORT["panel_message"] if not _ENV_READY else _DESCRIPTION_READY,
		"warning_message": _ENV_REPORT["warning_message"] if not _ENV_READY else "",
		"notice": _ENV_REPORT["warning_message"] if not _ENV_READY else "",
		"install_cmd": _ENV_REPORT.get("install_cmd", "") if not _ENV_READY else "",
		"copy_text": _ENV_REPORT.get("copy_text", "") if not _ENV_READY else "",
		"copy_label": _ENV_REPORT.get("copy_label", "") if not _ENV_READY else "",
		"model_download_url": _ENV_REPORT.get("model_download_url", "") if not _ENV_READY else "",
		"notice_level": _ENV_REPORT.get("notice_level", "ok"),
		"dependencies": [
			"GJJ/vendor/sam3 内置运行库：必须包含 __init__.py、predictor.py、utils.py 等文件。",
			"timm：SAM3 模型结构依赖。",
			"torchvision：图像预处理与模型算子依赖。",
			"opencv-python：图像处理运行依赖。",
		],
		"models": ["models/sam3/sam3.safetensors 或同目录下可被模糊匹配的 SAM3 权重。"],
	}
	...
	def segment(
		...
		try:
			...
			if needs_sam3:
				...
				try:
					...
					if report is not None:
						try:
							from .common_utils.dependency_checker import send_dependency_model_notice
			...
			# SAM3 运行环境缺失时，复用公共依赖面板而不是私有错误面板。
			if getattr(exc, "gjj_report", None) is not None or (isinstance(exc, RuntimeError) and "SAM3" in str(exc)):
				report = getattr(exc, "gjj_report", None) or build_sam3_environment_report(str(sam3_model or DEFAULT_SAM3_MODEL), original_error=str(exc))
				try:
					from .common_utils.dependency_checker import send_dependency_model_notice
					send_dependency_model_notice(report, unique_id=unique_id)
				except Exception:
					pass
				raise RuntimeError(report.get("warning_message") or "运行时依赖缺失：SAM3。详细信息请查看节点前端面板。") from exc

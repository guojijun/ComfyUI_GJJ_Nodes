...
10. **GJJ 节点必须同时满足"隐藏控件无空白 + 序列化往返正确"两个需求。** ⚠️ 最关键规则：下面 6 步缺一不可，只做部分会引入新 bug。

    **① import GJJ_Utils 并定义 HIDDEN_NAMES**
    ```javascript
    import { GJJ_Utils } from "./gjj_utils.js";
    const HIDDEN_NAMES = new Set(["internal_widget_1", "internal_widget_2"]);
    ```

    **② 用 GJJ_Utils.hideWidget 隐藏控件（-4 偏移零占位，不产生空白行）**
    ```javascript
    function compactNode(node) {
        GJJ_Utils.hideWidget(GJJ_Utils.getWidget(node, "widget_1"));
        GJJ_Utils.hideWidget(GJJ_Utils.getWidget(node, "widget_2"));
        GJJ_Utils.removeHiddenInputSockets(node, HIDDEN_NAMES);
        GJJ_Utils.reorderWidgets(node, HIDDEN_NAMES);
        GJJ_Utils.refreshNode(node);
    }
    ```
    > 必须用 `GJJ_Utils.hideWidget`（内置 `getHeight: () => -4` / `computeSize: () => [0, -4]`）。本地手写 hideWidget 用 `[0,0]` / `0` 会产生残留空白行。

    **③ 核心数据写入 serializedNode.properties（不受 widget 重排和输入槽删除影响）**
    ```javascript
    nodeType.prototype.onSerialize = function (serializedNode) {
        const result = originalOnSerialize?.apply(this, [serializedNode]);
        serializedNode.properties = serializedNode.properties || {};
        serializedNode.properties["widget_1"] = findWidget(this, "widget_1")?.value || "";
        serializedNode.properties["widget_2"] = findWidget(this, "widget_2")?.value || "";
        // 如有动态输出口，此处先修剪
        syncOutputs(this, rows, cols);
        return result;
    };
    ```

    **④ onConfigure 从 properties 恢复（覆盖 widgets_values 的错位值）**
    ```javascript
    nodeType.prototype.onConfigure = function (...args) {
        const result = originalOnConfigure?.apply(this, args);
        const props = this.properties || {};
        ["widget_1", "widget_2"].forEach((key) => {
            const w = GJJ_Utils.getWidget(this, key);
            if (w && props[key]) { w.value = props[key];
        w.callback?.(props[key]); }
        });
        compactNode(this);
        // 二次确保 + 创建 DOM
        setTimeout(() => { compactNode(this);
        createDomWidget(this); }, 80);
        setTimeout(() => compactNode(this), 120);
        return result;
    };
    ```

    **⑤ 所有 Widget 查找必须用 GJJ_Utils.getWidget，禁止本地 findWidget**
    ```javascript
    // 全文件统一替换：findWidget(node, "xxx") → GJJ_Utils.getWidget(node, "xxx")
    ```

    **⑥ Python RETURN_TYPES 声明的长度必须与 return 元组一致**
    ```python
    result = (batch,) + tuple(active_blocks)
    padding = (1 + MAX_BLOCKS) - len(result)
    if padding > 0:
        result += (empty_tensor,) * padding
    ```

    > **为什么这 6 步必须一起做？**
    > - 只做 ② 不做 ③④ → 控件隐藏了但保存/加载丢数据
    > - 只做 ③④ 不做 ② → 数据能保存但隐藏控件撑出空白行
    > - ②③④ 都用但本地手写 hideWidget 不用 GJJ_Utils → 空白行（`[0,0]` vs `[0,-4]`）
    > - 用 GJJ_Utils.compactNode 但不做 ③④ → removeHiddenInputSockets 删除输入槽 + reorderWidgets 打乱顺序 → widgets_values 索引错乱 → 加载数据错位
    > - 做 ③④ 但手写 findWidget 不用 GJJ_Utils.getWidget → 多处重复实现，维护不一致
    > - ⚠️ **顶部隐藏控件（按钮行之前）即使 `-4` 也残留空行** → 不创建 widget，Python 放 `hidden`，状态纯存 `properties` + `onSerialize`/`onConfigure` 持久化。详见 ComfyUI-Skills-Docs §8.10。
